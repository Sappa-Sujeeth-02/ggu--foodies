...
import LoadingPage from './LoadingPage'; // Assuming LoadingPage is in the same directory or adjust the path
...
const HomePage = () => {
    ...
    const [initialLoading, setInitialLoading] = useState(true); // New state for initial fetch
    ...
    const fetchData = async (isInitialFetch = false) => {
        if (isInitialFetch) {
            setInitialLoading(true); // Only set for initial fetch
        }
        try {
            ...
            if (isInitialFetch) {
                setInitialLoading(false); // Only clear for initial fetch
            }
        }
    };
    ...
    useEffect(() => {
        const fetchUserProfile = async () => {
            try {
                ...
                setInitialLoading(false); // Ensure initialLoading is false after profile fetch
            }
        };
        fetchUserProfile();
    }, []);
    ...
    useEffect(() => {
        if (location.state?.scrollToItemId && foodItemRefs.current[location.state.scrollToItemId]) {
            ...
            navigate(location.pathname, { replace: true, state: { selectedCourt } });
        }
    }, [location.state, navigate, selectedCourt]);
    ...
    const handleItemClick = (item) => {
        ...
        if (court) {
            ...
            setSelectedCategory('All Categories');
            setErrorMessage('');
            navigate('/home', { state: { selectedCourt: court, scrollToItemId: item._id } });
        }
    };
    ...
    const handleAddItem = async (item) => {
        ...
        if (cartItems.length > 0) {
            const existingCourt = cartItems[0].restaurant; // Fixed index syntax
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="relative">
                                ...
                                >
                                    ...
                                    <span className="text-gray-600 text-sm font-medium hidden md:inline">{userName}</span>
                                </button>
                                <AnimatePresence>
                                    {isProfileOpen && (
                                        <motion.div
